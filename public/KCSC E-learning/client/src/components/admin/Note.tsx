...
import { createNote, viewContentCreated } from '../../utils/admin/api';
...
const Note = () => {
  ...
  const onSubmit = async (data: any) => {
    try {
      if(currentUser) { const res = await createNote(data.content, currentUser.token);
        if (res === 200){
        ...
        <div className="flex w-full !max-w-[420px] shrink-0 flex-col items-center">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="mt-5 w-full">
            ...
            <button
              ...
              className="hidden h-10 w-20 mx-auto px-2 items-center justify-center mt-3 rounded bg-green-400 py-2 font-medium transition ease-in hover:bg-d-brand-hover"
            >
              See
            </button>
